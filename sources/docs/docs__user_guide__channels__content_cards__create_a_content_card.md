---
url: https://www.braze.com/docs/user_guide/channels/content_cards/create_a_content_card
slug: docs__user_guide__channels__content_cards__create_a_content_card
title: "Create a Content Card"
...
- Select Content Cards or, for campaigns targeting multiple channels, select Multichannel.
...
- Add and name as many variants as you like for your campaign. You can choose different platforms, message types, and layouts for each of your added variants. For more on variants, see Multivariate and A/B testing.
...
- Filter your audience, or the recipients, for this step as necessary in the Delivery Settings. You can further refine your audience by specifying segments and adding additional filters. Audience options are checked after the delay, at the time messages are sent.
...
To learn more about the expected behavior and look of each type, see Creative Details, or check out the links in the following table. These Content Card types are accepted by both mobile apps and web applications.
 ...
 Classic | 
 | 
 The Classic Card has a straightforward layout with a bolded title, message text, and an optional image that sits to the start of the title and text. It’s best to use a square image or icon with the Classic Card. | 
 ...
 Image Only | 
 | 
 The Image Only Card commands attention with space for images, GIFs, and other creative non-text content. | 
...
Select Add Languages to add your desired languages from the provided list. This inserts Liquid into your message. We recommend selecting your languages before writing your content so you can fill in your text where it belongs in the Liquid. For our full list of available languages you can use, see Languages supported.

#### Create right-to-left messages

The final appearance of right-to-left messages depends largely on how service providers render them. For best practices on crafting right-to-left messages that display as accurately as possible, see Creating right-to-left messages.
...
The following actions are available for Content Card links:
 ...
 Log Custom Event | 
 Choose a custom event to trigger. Can be used to display another Content Card or trigger additional messaging. | 
...
Content Cards can be delivered based on a scheduled time, an action, or an API trigger. For more, see Scheduling your campaign.

You can also set the campaign’s duration and Quiet hours and determine the Content Card’s expiration. Set a specific expiration date or the days until a Card expires, up to 30 days. All variants must use the same expiration (duration or specific time).
...
For action-based delivery, there is an expected short delay before the Content Card appears. For details on why this happens and how to minimize it, see Why don’t Content Cards appear immediately after a trigger event?.

#### Scheduled delivery

For Content Card campaigns with scheduled delivery, you can choose when Braze evaluates audience eligibility and personalization for new Content Card campaigns by specifying when the card is created. For more, see card creation.
...
After you finish building your campaign or Canvas, review its details, test it, then send it. For more information, see Send test messages.
...
- Stop the original campaign and, when prompted, select Remove card after the next sync.
 
- Duplicate the campaign, make your edits, and launch the new version.
...
- Stop your campaign and, when prompted, select Remove card after the next sync.
...
- Open the Content Card campaign and select Stop Campaign.
 
- When prompted, select Remove card after the next sync. The card is removed on the next feed refresh.
...
Content Cards sync on a schedule and when your app refreshes the feed. Sync behavior differs between full and partial syncs, and your SDK integration affects when cards refresh at session start. For implementation details, see Customize the Content Card feed and Creating Content Cards.
...
This approach ensures existing users see cards instantly while still reaching new users after a brief delay in their first session. For additional strategies to improve latency, see Improve low latency for Content Cards.
