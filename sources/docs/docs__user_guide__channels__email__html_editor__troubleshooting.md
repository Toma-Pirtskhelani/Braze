---
url: https://www.braze.com/docs/user_guide/channels/email/html_editor/troubleshooting
slug: docs__user_guide__channels__email__html_editor__troubleshooting
title: "Troubleshoot HTML emails"
...
Match your symptom in the table to navigate to the relevant section.
 ...
 Test email HTML looks wrong | 
 HTML renders incorrectly in test emails | 

 Editor behaves oddly in Chrome | 
 Extension conflicts | 

 Email looks different across clients | 
 Email rendering | 

 Email displays Liquid code or broken links | 
 Unbalanced HTML in Liquid templates | 

 Inbox Vision preview doesn’t match sent email | 
 CSS inlining | 

 White space or lines after images in test emails | 
 White space under images | 

 Click analytics don’t include query parameters | 
 Link click analytics limitations | 

 Superscripts cause inconsistent line spacing | 
 Superscript line height issues | 
...
- Validate your HTML markup in the editor or an external validator.
 
- Send a test email and note which email clients or browsers show the issue.
 
- Preview with Inbox Vision to compare rendering across clients.
 
- Rule out browser extension conflicts if the editor itself misbehaves.
 
- If the issue persists, open a support ticket with screenshots from Inbox Vision and the affected clients.

## HTML renders incorrectly in test emails

### Symptom

A test email doesn’t match what you expect from the editor.
...
- If the issue is specific to how alt text displays, keep in mind that this behavior is controlled by the recipient’s email client, not Braze.
...
User-level click data remains available through Currents or the Message Activity Log, regardless of how many unique parameterized links are clicked.
