---
url: https://www.braze.com/docs/user_guide/brazeai/operator/data_privacy_security
slug: docs__user_guide__brazeai__operator__data_privacy_security
title: "Data privacy and security for BrazeAI Operator"
...
BrazeAI OperatorTM integrates with OpenAI to provide AI-powered assistance. This article covers how Operator handles data, what information is shared with OpenAI, and how to minimize PII exposure and control access.
...
How long OpenAI retains data sent through Operator depends on your cluster:
 ...
 US-02 (HIPAA customers) | 
 Zero Data Retention (ZDR). Data is not stored by OpenAI after processing. | 

 All other clusters | 
 30 days for abuse monitoring. This is an industry-standard retention period imposed by OpenAI. | 
...
If a user does not have the “View PII” permission, Operator cannot surface PII to them. Keep in mind that Operator does not filter content typed directly into prompts—PII entered manually is sent to the underlying language model. To reduce this risk, see Minimize PII exposure.

### Audit team usage

Download Braze’s Security Event Report to monitor team usage. The “Requested BrazeAI Operator Response” event provides a comprehensive audit trail, allowing you to review the exact inputs provided to the Operator.
