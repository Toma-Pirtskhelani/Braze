---
url: https://www.braze.com/docs/user_guide/messaging/messaging_fundamentals/accessibility
slug: docs__user_guide__messaging__messaging_fundamentals__accessibility
title: "Build accessible messages in Braze"
description: "This reference article explains why accessibility is important in marketing content, how Braze accessibility language (HTML lang) works across channels, and how you can build..."
...
Understand why accessibility is important to consider in your marketing content, and how you can build accessible messages in Braze. For more guidance, check out our Accessible Messaging Foundations course on Braze Learning.
...
This section is partially adapted from W3C: Diverse Abilities and Barriers.
...
- Globally, at least 2.2 billion people have a near or distance vision impairment (see WHO)
 
- Around 1 in 12 men and 1 in 200 women have some degree of color vision deficiency, an estimated 300 million people in the world (see NHS)
...
- Approximately 15% of American adults (37.5 million) aged 18 and over report some trouble hearing (see NIH)
...
Almost 2 million people in the US live with limb loss (see Amputee Coalition)
...
- One in five people in the United States have learning and attention issues (see LDA)
 
- Roughly 10-20% of the global population is considered neurodivergent (see Deloitte)
 
- About 1 in 100 children has autism worldwide (see WHO)
...
- Avoid justified text: Justified text creates uneven spacing between words, making reading difficult for people with dyslexia or cognitive disabilities. Consider making content that wraps to more than two lines aligned left for left-to-right languages or aligned right for right-to-left languages.
...
- Clearly label links and buttons: Make sure your link guidance and button text clearly explains what happens next. It helps people using screen readers or navigating with a keyboard know what to expect.
...
- Test for truncation: Always test your copy by sending a test message to a device to make sure your text isn’t truncated. If your message is being cut off, this hurts both you and your audience, since it prevents your content from reaching them.
...
When your editor includes it, go to the Accessibility section in message settings. Pick a language from the dropdown or use Liquid (for example {{accessibility_language}} when multi-language messages are turned on and Localization Settings are set).

#### Multi-language messages

In Localization Settings, set an accessibility language for each locale so Liquid can fill in {{accessibility_language}} for localized sends. Whether that value is already chosen for new messages depends on the channel. For CSV and translation workflows, start with Language settings and accessibility.
...
Use this table to compare channels. Defaults can differ, so spot-check what your audience actually receives.
 ...
 Email (drag-and-drop, full template) | 
 Set language in the editor. With multi-language messages, a full email template can match each locale’s language for you. If you only use Content Blocks (single row), those shortcuts don’t work the same—pick the language yourself where the editor allows it. | 

 Email (HTML code) | 
 Braze does not add a language tag for you. Add it in your HTML if you need it. | 

 In-app messages (drag-and-drop) | 
 When you pick a language under Accessibility, Braze adds that language to the outer HTML of the message so screen readers treat the whole message as that language. With multi-language messages on, new messages may default to your locale languages. Preview might show no language until you choose one under Settings. | 

 Banners | 
 Same behavior as in-app messages. | 

 Landing pages | 
 You can set language on the live page. Choose one language, or use Liquid if your account allows Liquid on landing pages. Defaults also differ from in-app messages and Banners—check the published page. | 

 Content Cards | 
 Cards use a Language field for apps instead of an explicit accessibility language. | 

When you write HTML yourself, you can still add a language tag on part of the message (for example one phrase in another language). For more patterns, see Custom HTML.

#### Standards reference

When Braze adds a root-level language tag to HTML, it follows the HTML lang rule. Testing tools often look for html-has-lang. Content Cards use your language field instead of that HTML pattern.

### Buttons

Use buttons to indicate an action, such as sending a form or playing a carousel. If you’re navigating to a new URL, consider using a link guidance instead.
...
Button text must be easy to read against the button’s background color. Check that your button text meets WCAG 2.2 AA contrast minimums:

- 4.5:1 contrast ratio for normal-sized text (most buttons)
 
- 3:1 contrast ratio for large text (typically 18pt or larger)

High contrast helps buttons remain readable and clickable for everyone, including users with visual impairments or those viewing your message in challenging environments. For more guidance, see the Color contrast section.

#### Make buttons easy to tap

Make sure your buttons (and links) are big enough and spaced far enough apart for users on mobile devices. Small or crowded touch targets can be frustrating or impossible for users with motor disabilities to interact with.
...
For example, consider how you might write a link to view a weather report.
 ...
 Click here | 
 Click here to access today’s weather | 
 Today’s weather | 
...
Braze drag-and-drop editors output semantic HTML by default, so links aren’t styled like buttons there. However, if you’re working with custom HTML or making code-level changes, keep this in mind:

- Links (<a>) respond to the Enter key.
 
- Buttons (<button>) respond to both the Enter and Space keys.

Styling a link to look like a button can confuse people who navigate with a keyboard—they might try pressing Space and expect it to work.
...
- Use adequate touch target size. Aim for a minimum touch target size of 44 x 44 pixels. This aligns with WCAG 2.2 guidelines for touch targets and common mobile usability standards.
...
- Test for readability and contrast: If you must keep text in the image, follow color contrast best practices and use a large scale font. This means text should be at least 18 points (about 24 pixels) for non-bold text or 14 points (about 18 pixels) if it’s bold. Using these sizes helps text remain legible without forcing users to zoom in, and it improves the overall contrast and readability of the content. Test to confirm it’s still legible on smaller screens.
...
- Reflect text that appears in the image
 
- Stick to relevant context—no extra marketing jargon
...
Screen reader users rely on alt text to understand the content or function of an image. Avoid generic “marketing speak” that doesn’t match what’s visually shown.
 ...
 "Smiling woman wearing a blue denim jacket, holding a shopping bag." | 
 "Time to treat yourself!" (No mention of what's actually in the image) | 

 "Man wearing a black T-shirt, leaning on a bike in a city street." | 
 "Embrace your best life now!" (Ignores the bike and city setting) | 

 "Blue apartment building with a 'For Rent' sign in front." | 
 "The key to a better tomorrow!" (Doesn't reflect the apartment or sign) | 

##### Keep it short, yet specific

Concise alt text makes it easier for users to process. Include enough detail to convey purpose but skip any fluff. As a general rule, keep alt text to 125 characters or less. If anything more than a brief phrase or sentence is necessary, consider using one of the long description methods from W3C.

 Good examples ✅ | 
 Poor examples 🚫 | 

 "Red running shoes on a white background" | 
 "Running shoes that are extremely comfortable and perfect for your active lifestyle in a vibrant shade of red." (Too long and filled with promotional fluff) | 

 "Four laptops on a display stand" | 
 "Discover the ultimate productivity booster that redefines how you work every day, in every way imaginable." (Doesn't describe what's actually shown) | 

 "Group of friends eating ice cream on a sunny day" | 
 "Capture pure happiness with the sweetest treat—life is better with our brand of ice cream!" (Too abstract and brand-focused) | 
...
Screen readers already announce an image. Jump right into describing the subject.
 ...
 "Table set for brunch with pancakes, fruit, and coffee." | 
 "Image of a table set for brunch" | 

 "Roadside billboard with bold 'Grand Opening' text" | 
 "Picture of a billboard on the side of a road" | 

 "Snowy mountain landscape at sunset" | 
 "Photo of snow and mountains" | 
...
If an image includes essential text, put that information in the alt text so users don’t miss it.
 ...
 "Banner reading ‘Summer Sale—50% off all swimwear.'" | 
 "Banner promoting a sale." (Fails to mention the actual discount) | 

 "Logo with the text ‘Café Toscana' in script font" | 
 "Logo image for a café." (Doesn't include text ‘Café Toscana') | 

 "Ad announcing ‘Concert Tickets Available Now—Starts June 5th'" | 
 "Concert ad." (No event details) | 
...
Don’t pad alt text with SEO terms or calls to action not directly related to the image. Provide value for those who can’t see the image.
 ...
 "Laptop showing the Braze dashboard analytics chart" | 
 "Boost conversions and skyrocket ROI with the best platform on earth!" (Adds unnecessary marketing language) | 

 "Backyard patio set featuring four chairs and a glass table" | 
 "Host an incredible summer party for all your friends and family now!" (Describes a scenario, not the image) | 

 "Mobile phone displaying a weather forecast app with 75°F in view" | 
 "Experience real-time innovations in weather tracking that's a game-changer" (Doesn't reflect what's visibly shown) | 
...
If an image is functioning like a link or call-to-action, describe the intended action (“Shop,” “Link to,” “Sign up”), not just the label or product shown.
 ...
 "Fall Collection" (Missing intended action) | 

 "Link to free eBook" | 
 "Free eBook" (Doesn't make it clear this is a link) | 

 "Sign up for the mailing list" | 
 "Mailing list" (Doesn't describe what the user can do) | 
...
While you can include alt text of any length in your email HTML, some email clients may truncate or hide alt text that is too long to fit within the image’s dimensions. If you have questions about alt text behavior in a specific email client, contact Support.
...
Sufficient color contrast helps ensure your messages are easy to read for everyone, including people with low vision or those viewing your content in bright or challenging conditions. Aim for contrast ratios that comply with WCAG 2.2 AA level requirements:
...
You can test your color choices using the WebAim Contrast Checker Tool.
...
- Use semantic HTML. This means using the correct HTML elements for their intended purpose instead of styling one element to look like another. Most HTML elements have their own accessibility support built in.
 
- For document-level language where Braze can add HTML metadata on export, refer to Accessibility language; behavior varies by channel. When you mark up content yourself, set the lang attribute within your HTML to identify the language that your content is in. Screen readers use different sound libraries for each language based on the pronunciation and characteristics of that language. If this isn’t specified, a screen reader assumes the content is written in the default language that the user chose when setting up the screen reader. If the message isn’t actually in the default language, then the screen reader might not pronounce the message correctly.
...
When using the drag-and-drop email editor, set language from the Settings tab when that control is available. Full-template and content-block-only email can use different defaults for accessibility language—see Accessibility language. Other channels are covered in that section too.

- Use ARIA attributes to give extra context. These attributes provide additional information to assistive technologies, helping to clarify the role, state, or properties of UI elements that may otherwise be unclear.

### ARIA attributes

When you’re using custom code in Braze editors, you can use Accessible Rich Internet Applications (ARIA) to provide extra accessibility support for users who rely on assistive technology. ARIA roles and attributes help screen readers interpret your content more clearly, especially when you’re using elements that don’t convey meaning on their own (like <div> or <span>).
...
In general, it’s better to use alt="" for decorative images and icons rather than aria-hidden="true". While semantic HTML is widely supported by all screen readers and assistive software, ARIA support varies. Even if you use aria-hidden you should still include an empty alt attribute.
...
- Inbox Vision for emails
 
- Accessibility scanner for messages created using our HTML editor (for example, HTML in-app messages, HTML Content Blocks, custom email footers, email opt-in pages, and email unsubscribe pages).

These tests check your message against the Web Content Accessibility Guidelines (WCAG) standard—a set of internationally recognized technical standards for accessible content. Any issues that can be detected automatically are flagged and categorized by severity to help you prioritize.
