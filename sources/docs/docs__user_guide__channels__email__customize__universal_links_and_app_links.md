---
url: https://www.braze.com/docs/user_guide/channels/email/customize/universal_links_and_app_links
slug: docs__user_guide__channels__email__customize__universal_links_and_app_links
...
For a comparison of link types across all messaging channels and guidance on when you need an AASA file, see iOS deep linking guide.
...
Firebase Dynamic Links is deprecated. Braze does not have a direct integration with Firebase, and deep linking is managed outside the Braze platform. Migrate to platform-native solutions (Apple universal links and Android App Links, as described in this article) or to alternative deep linking service providers. For migration guidance, see Firebase’s migration FAQ.
...
This table outlines the key differences between universal links and traditional deep links:
 ...
 iOS (version 9 and later) and Android (version 6.0 and later) | 
 Used in various mobile OS | 

 Purpose | 
 Seamlessly link web and app content on iOS and Android devices | 
 Link to specific app content | 

 Function | 
 Directs to web pages or app content based on context | 
 Opens specific app screens | 

 App Installation | 
 Opens app if the app is installed, otherwise opens web content | 
...
These steps are adapted from the Apple developer documentation. For more information, refer to Allowing apps and websites to link to your content.

### Step 1: Configure your app entitlements

note

In Xcode 13 and later, Xcode can handle entitlement provisioning for you automatically. You can likely skip to step 1c and refer back to these instructions if you have issues.

#### Step 1a: Register your app

- Go do developer.apple.com and log in.
 
- Click Certificates, Identifiers & Profiles.
 
- Click Identifiers.
 
- If you don’t have a registered App Identifier yet, click + to create one.
...
- In your existing or newly created App Identifier, locate the App Services section.
 
- Select Associated Domains.
 
- Click Save.

#### Step 1c: Turn on Associated Domains in your Xcode project
...
- Check that your Bundle ID (step 1a) of your Xcode project matches the one used to register the App Identifier.
...
When a user taps a universal link on an iOS device, the device launches the app and sends it an NSUserActivity object. The app can then query the NSUserActivity object to determine how it was launched.
...
These steps are adapted from the Android developer documentation. For more information, refer to Add Android App Links and Create Deep Links to App Content.
...
First, you need to create deep links for your Android app. This can be done by adding intent filters in your AndroidManifest.xml file. The intent filter should include the VIEW action and BROWSABLE category, along with your website’s URL in the data element.
...
- Set up your AASA or AndroidManifest pathPrefix values to only treat links with /uni/ in the URL path as universal links.
 
- Add the attribute universal="true" to your link’s anchor tag (<a>). This changes the URL path of the wrapped link to include /uni/.
...
- Make sure your app is set up to handle the wrapped links properly. Refer to SendGrid’s article on Resolving SendGrid Click Tracking Links and follow the steps for your operating system. This article contains example code for iOS and Android.
...
Then, make sure your app is set up to handle the custom path properly. Refer to SparkPost’s article on Using SparkPost click tracking on deep links. This article contains example code for iOS and Android.
...
- Add ses:custom-path attributes to your anchor tags in email HTML, or add the attribute in the Attributes section of the drag-and-drop editor for email. The custom path is inserted into the wrapped click-tracking URL.
...
- Confirm your wrapped tracking URLs include the custom path segment. Without the attribute, tracked links use track.yourstore.com/CL0/{encodedUrl}/.... With the attribute, they follow this format: track.yourstore.com/CL1/{customPath}/{encodedUrl}/...
...
- Configure your site association files on your click-tracking domain so paths match /CL1/{customPath}/.
...
- iOS: Review the Associated Domains set up in Xcode for your app (Step 1c: Turn on Associated Domains in your Xcode project). Check that the click-tracking domain is included in that list.
...
Limit those patterns to the URLs that should open the app. For SendGrid, match /uni/ and add universal="true" only on those links. See Universal links, App Links, and click-tracking.
...
- Go to Settings > Email Preferences in the Braze dashboard.
 
- Review the Outbound Email Settings under Sending Configuration.
 
- Confirm that your sending domain and tracking domain are properly aligned for the workspace where links aren’t working.
