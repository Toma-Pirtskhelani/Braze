---
url: https://www.braze.com/docs/user_guide/brazeai/predictive_suite/predictive_events/use_case
slug: docs__user_guide__brazeai__predictive_suite__predictive_events__use_case
title: "Use case: Predict subscription upgrades with smarter targeting"
description: "This example shows how a fictional brand uses Braze Predictive Events to define the outcomes that matter to their business—like upgrading to a pro membership—and..."
...
- In the Braze dashboard, Jordan goes to Analytics > Predictive Events.
 
- He creates a new event prediction and names it “Upgrade to Pro in 7 days”
 
- As the target event, he selects his custom event: upgraded_to_pro.
 
- Jordan sets the prediction window to 7 days, sets an update schedule, and creates the prediction.

## Step 2: Segment users based on upgrade probability

After training is complete, Braze assigns an Event Likelihood Score (0-100) to each eligible user. Jordan uses this score to create actionable segments—one for high-intent users who may not need a discount, and another for users who likely won’t convert without support.

- Jordan navigates to Segments in Braze.
 
- He creates two segments using the Event Likelihood Score filter and selects the prediction he created. The two segments are:
...
Jordan creates a Canvas that splits the experience based on the segments he just built. He adds an Audience Paths step to target:
...
After the campaign runs, Jordan reviews performance in Canvas Analytics to understand how well the personalized paths performed—and whether combining predictive intent with behavioral signals improved upgrade rates.
...
The funnel report also shows a clear reduction in drop-off across key steps, particularly for low-intent users who received personalized messaging. More users are opening, clicking, and upgrading—proving the value of intent-based targeting.
