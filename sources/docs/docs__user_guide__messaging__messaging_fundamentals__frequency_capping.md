---
url: https://www.braze.com/docs/user_guide/messaging/messaging_fundamentals/frequency_capping
slug: docs__user_guide__messaging__messaging_fundamentals__frequency_capping
title: "Rate limiting and frequency capping"
...
- User-centric rate limiting: Focuses on providing the best experience for the user.
 
- Delivery speed rate limiting: Takes into consideration the bandwidth of your servers.
...
Set a workspace messaging rate limit to enforce a rate limit across an entire workspace.
...
Updates to rate-limiting interface
...
- Existing campaigns and Canvases: All existing campaigns and Canvases have been migrated to this interface. Their delivery behavior remains the same. The dashboard displays whether the campaign uses shared or per-channel logic.
...
- Content Cards configured to create “at first impression” (This will be controlled by the rate of app impressions. Refer to Card creation for more information on the differences between Card Creation options.)

- Delivery rate speed limits aren’t supported for the following:

- SMS autoresponses
 
- SLA-backed messages (such as Transactional Email)
...
- Rate limit is applied first to select the initial pool of users who can receive messages.
 
- Frequency cap is applied second to filter users from that pool.
 
- Messages are sent to the remaining users.
...
- How many times each user should receive a campaign or Canvas component sent by tag within a certain time frame.
...
Global frequency capping alone doesn’t exit users from a Canvas. On Message steps, users still advance when a message isn’t sent because of global frequency capping, in line with how users advance through the step. The same applies when a message is aborted (for example, by a Liquid abort condition): the user continues through the Canvas as if the message had been sent.
...
When sending API campaigns, which are often transactional, you’ll have the ability to specify that a campaign should ignore frequency capping rules by setting override_frequency_capping to true in the API request.
...
Frequency capping rules can be applied to workspaces using specific tags you have applied to your campaigns and Canvases, allowing you to essentially base your frequency capping on custom-named groups.
...
- No more than three push notification campaigns or Canvas components per week from all campaigns and Canvas steps. 
AND
 
- No more than two push notification campaigns or Canvas components per week with the tag promotional.
...
- No more than one push notification campaign or Canvas component per week from all campaigns and Canvas components. 
AND
 
- No more than three push notification campaigns or Canvas components per week with the tag promotional.
...
- Your user should receive no more than one push notification campaign per week with the tag promotional.

 Action | 
 Result | 

 The promotional tag is removed from Campaign A after your user received the message, but before Campaign B has sent. | 
 Your user receives Campaign B. | 

 The promotional tag is mistakenly removed from Campaign A after your user received the message. 
 The tag is added back to Campaign A on Tuesday, before Campaign B is sent. | 
 Your user does not receive Campaign B. | 
...
Users who are frequency capped don’t generate a send event for that step. To identify these users, you can use Currents to track message frequency capped events. Alternatively, you can create a Segment Extension to analyze users who entered the Canvas but didn’t receive the expected message.

### Why does the dashboard show a rate limit error for my campaign?

This usually means the campaign’s delivery speed rate limit is set too low for the audience size, so completing the send would take longer than the allowed window and Braze surfaces a warning. Increase the delivery speed rate limit, reduce the audience, or use Limit send volume so each scheduled occurrence finishes within the allowed send window. You can also set a workspace messaging rate limit to enforce a cap across campaigns.
...
If messages are temporarily deferred, sending less may help in the short term. Use a delivery speed rate limit, Limit send volume, or both.
