---
url: https://www.braze.com/docs/developer_guide/getting_started/sdk_overview
slug: docs__developer_guide__getting_started__sdk_overview
title: "SDK overview for developers"
description: "This onboarding reference article provides a technical overview for developers of the Braze SDK. It discusses default analytics tracked by the."
...
Are you a marketer looking for a basic rundown of the SDK? Check out our marketer overview, instead.
...
If necessary for your use case, you can limit the collection of certain data after the integration is complete.
...
You can use setDeviceObjectAllowlist to configure the SDK to only send a subset of the device object keys or values according to a set allowlist. This must be enabled via setDeviceObjectAllowlistEnabled.

important

An empty allowlist will result in no device data being sent to Braze.

You can assign a set of eligible fields to configuration.devicePropertyAllowList on your Braze.Configuration to specify an allowlist for device fields that are collected by the SDK. The full list of fields is defined in Braze.Configuration.DeviceProperty. To turn off the collection of all device fields, set the value of this property to an empty set ([]).

important

By default, all fields are collected by the Braze Swift SDK. Removing some device properties may disable SDK features.

For more usage details, refer to Storage in the Swift SDK documentation.
...
If you have an iOS app, you can confirm that you are using the Swift SDK instead of the legacy Objective-C iOS SDK if your Live SDK Version is equal to or higher than 5.0.0, which was the first released version of the Swift SDK.
