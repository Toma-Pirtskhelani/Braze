---
url: https://www.braze.com/docs/user_guide/data/activation/events/custom_events
slug: docs__user_guide__data__activation__events__custom_events
title: "Custom events"
...
This article describes custom events and properties, user profile event history, related segmentation filters, Canvas entry properties, relevant analytics, and more. To learn about Braze events in general, refer to Events.
...
There is no fixed dashboard cap on how many distinct custom events or custom attributes you can define or store on a profile; practical limits depend on data shape, ingestion volume, and workspace performance. If you plan to track a very large catalog of events or attributes, work with your Braze account team on modeling and hygiene (for example, blocklisting unused data).
...
- Go to Audience > Search Users, then select a user to open their profile.
...
Viewing the Event History tab requires the Search Users, View PII, and View User Event Properties user permissions because event properties can contain personal data. For more information, refer to Company user permissions.
...
Braze notes the number of times custom events have occurred and the last time they were performed by each user for segmentation. For report setup, filters, and export options, see Custom events report.
