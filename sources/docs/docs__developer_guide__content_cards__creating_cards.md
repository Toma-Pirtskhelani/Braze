---
url: https://www.braze.com/docs/developer_guide/content_cards/creating_cards
slug: docs__developer_guide__content_cards__creating_cards
...
Content Cards only refresh on session start if subscribeToContentCardsUpdates() is called before openSession(). You can also manually refresh the feed at any time.
...
Additionally, if you want to maintain a subscription to your content cards, you can call subscribeToUpdates:
...
- Set an active user in your application by calling the changeUser() method.
 
- In Braze, go to Campaigns, then create a new Content Card campaign.
 
- In your campaign, select Test, then enter the test user’s user-id. When you’re ready, select Send Test. You’ll be able to launch a Content Card on your device shortly.
