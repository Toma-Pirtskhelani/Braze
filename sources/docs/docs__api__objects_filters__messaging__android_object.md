---
url: https://www.braze.com/docs/api/objects_filters/messaging/android_object
slug: docs__api__objects_filters__messaging__android_object
title: "Android object"
...
The android_push object allows you to define or request information related to Android Push and Android Push Alert content through our messaging endpoints.
...
### Additional parameter details
 ...
 priority | 
 This parameter accepts values from -2 to 2, where -2 represents “MIN” priority and 2 represents “MAX”. 0 is the “DEFAULT” value. 
 
 Any values sent outside of that range default to 0. For more information on which priority level to use, see Android notification priority. | 

 android_priority | 
 This parameter accepts values either normal or high to specify the FCM sender priority. By default, messages are sent with the default FCM priority configured in the Push Settings page.

 For more information on how different values impact delivery, see Android message priority. | 

 collapse_key | 
 FCM can simultaneously store up to only four collapse keys per device. If you use more than four collapse keys, FCM does not make any guarantees as to which keys are kept. Braze uses one of these by default for campaigns, so make sure to specify up to only three additional collapse keys for Android messages. | 

 push_icon_image_url | 
 The value for the large icon parameter should be a URL that links to where your image is hosted. 
 
 Images need to be cropped to a 1:1 aspect ratio and should be at least 40x40. | 

 notification_channel | 
 If this is not specified, Braze attempts to send the notification payload with the dashboard fallback channel ID. For more, see Notification channels and refer to the steps for defining notification channels during integration. | 
