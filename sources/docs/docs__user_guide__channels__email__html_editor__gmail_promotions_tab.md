---
url: https://www.braze.com/docs/user_guide/channels/email/html_editor/gmail_promotions_tab
slug: docs__user_guide__channels__email__html_editor__gmail_promotions_tab
...
The Gmail mobile Promotions tab allows marketers to send more information via annotations in a “card” rather than just the subject line or preheader information. Braze has a built-in tool to help you build the card from your email campaign.

## Prerequisites

First, forward your domains and subdomains to Google’s Promotions Tab outreach team at [email protected] to be added to Gmail’s allowlist. This allows you to use any feature that shows rich imagery, such as the product carousel for the Gmail Promotions tab.
...
First, create your email campaign, and select the HTML code editor as your editing experience.
...
- Select the Discount Offer toggle.
 
- For Offer, enter a short summary for the discount. An example is “20% off”.
 
- For Code, add the promotion code that a user needs to apply at checkout.
 
- Then, select the start date and time for the discount offer.
 
- Determine if the discount offer should end at a specific time or never end.
...
- Select the Deal Card toggle.
 
- For Offer, enter a short summary for the discount. An example is “20% off all shoes”.
 
- (optional) For Code, add the promotion code that a user needs to apply at checkout.
 
- Enter at least one of the following URLs.
...
- (optional) Add a start date for the offer.
 
- Determine if the offer should end at a specific time or never end.
...
- Select the Promotion Cards toggle.
 
- Select Add promotion card. Each image in your product carousel must have a unique URL and use the same aspect ratio (4:5, 1:1, 1.91:1).
 
- Include an image URL.
 
- For Target URL, add the link for your promotion.
...
- Customize your promotion card by adding a headline, currency, price, and discount value.

 Customizable property | 
 Description | 

 Headline | 
 (optional) One or two sentence description for the promotion. Displays under the preview image. | 

 Currency | 
 (optional) The currency of the price. | 
...
For the drag-and-drop editor, copy and paste the generated HTML code into the custom head tags section under Sending Settings.
...
To test annotations for low-volume sends, you must use Gmail’s preview tool to validate the annotations first. If you skip this step, the product carousel and single image preview will only trigger at higher send volumes.
...
In general, follow Gmail’s Promotions tab best practices.

As you build your card, consider the following questions:

- Is the annotation script valid? Preview with Google.
...
Use the preview tool to preview your annotation. Note that sending yourself a test email won’t work for annotations, as your annotation only renders if the email has to be sent to a significant number of recipients. Make sure you send the final email (with its image URLs) to at least 100 Gmail recipients.
...
Use Google Annotations to add your logo and promo card in the Gmail app. Rendering is controlled by Gmail, not Braze.
...
For more troubleshooting, see Google’s troubleshooting guide for email promotions.
