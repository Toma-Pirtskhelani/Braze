---
url: https://www.braze.com/docs/developer_guide/getting_started/integration_overview
slug: docs__developer_guide__getting_started__integration_overview
title: "Getting started: Integration overview"
...
- Channel (for example, push notifications or in-app messages)
 
- Delivery method (for example, scheduled delivery or action-based delivery)
...
For Web implementations that span multiple root domains, refer to Multi-domain integration for the Braze Web SDK when deciding whether to use one app or separate apps and API keys.
...
To add an additional layer of security, you can enable SDK Authentication to prevent unauthorized SDK requests. This feature is available across all major platforms including Web, iOS, Android, React Native, Flutter, Unity, Cordova, .NET MAUI (Xamarin), and Expo.
...
- Importing historical data; and
 
- Continuous updates that aren’t triggered in Braze. For example, a user profile upgrades to VIP without them logging into an app, so the API needs to communicate this info to Braze.
...
User IDs, called external_ids in Braze, should be set for all known users. These should be unchanging and accessible when a user opens the app, allowing you to track your users across devices and platforms. See the User lifecycle article for best practices.
...
Make sure your production and testing environments are set up before beginning QA.
 ...
 You’ll perform quality assurance on the way data is ingested, stored, and exported. | 
 You’ll make sure that your messages are being sent correctly to your users and everything looks excellent. | 

 Run tests to confirm data is stored properly. | 
 Create segments of users. | 

 Confirm session data is correctly attributed to the intended workspace within Braze. | 
 Launch campaigns and Canvases successfully. | 

 Confirm session starts and ends are being recorded. | 
 Confirm the correct campaigns are being shown to the correct user segments. | 

 Confirm user attribute information is correctly recorded against user profiles. | 
 Confirm that push tokens are correctly being registered. | 

 Test that custom data is being correctly recorded against user profiles. | 
 Confirm that push tokens are correctly removed. | 

 Create anonymous user profiles. | 
 Test that push campaigns are correctly sending to devices and engagement is logged. | 

 Confirm that anonymous user profiles become known user profiles when the changeUser() method is called. | 
