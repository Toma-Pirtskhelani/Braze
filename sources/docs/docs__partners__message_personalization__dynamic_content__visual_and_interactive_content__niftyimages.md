---
url: https://www.braze.com/docs/partners/message_personalization/dynamic_content/visual_and_interactive_content/niftyimages
slug: docs__partners__message_personalization__dynamic_content__visual_and_interactive_content__niftyimages
title: "NiftyImages"
...
NiftyImages helps Braze customers create personalized, real-time visual content for email, mobile, and in-app messaging. By connecting live customer, product, and business data to dynamic images and content, brands can deliver timely, relevant experiences such as countdown timers, personalized recommendations, localized messaging, inventory updates, and promotional offers that drive engagement and conversions.

This integration is maintained by NiftyImages.

## About the integration

The NiftyImages integration for Braze helps you create personalized, dynamic visuals using Braze contact data. Teams can build assets like personalized images, countdown timers, maps, calendars, loyalty visuals, and more, then publish them as reusable Braze Content Blocks for use across campaigns and Canvases. This saves time, reduces errors, and simplifies personalized content management.
...
Before you begin, confirm you have the following:
 ...
 NiftyImages account | 
 A NiftyImages account is required to create and manage personalized images, timers, maps, calendars, scratch-offs, charts, and other dynamic visuals. | 

 Braze account | 
 A Braze account is required to use NiftyImages within Braze campaigns, Canvases, email templates, and messaging channels. | 

 Braze REST API key | 
 A Braze REST API key with custom_attributes.get and content_blocks.create permissions.
...
- In NiftyImages, go to Settings > Integrations.
 
- Select Braze.
 
- Select Connect Braze.

### Step 2: Create your Braze REST API key

- In Braze, go to Settings > APIs and Identifiers.
 
- Create or select a REST API key for the NiftyImages integration.
 
- Under Custom Attributes, select custom_attributes.get.
 
- Under Content Blocks, select content_blocks.create.
 
- Save the API key, then copy the REST API key and your REST endpoint.

### Step 3: Connect your Braze account in NiftyImages

- Return to the Braze integration screen in NiftyImages.
 
- Paste the Braze REST API key.
 
- Enter your Braze REST endpoint.
 
- Confirm the connection.
 
- Verify your Braze account appears under Connected Braze accounts with an Active or Connected status.
...
- Create or edit a personalized image or other NiftyImages asset.
 
- Open the merge tag or personalization picker.
 
- Select Pick from connected integrations, then choose the Braze properties you want to use.
 
- Add those values to text, image, timer, map, chart, calendar, or dynamic content layers.
...
- Finish your NiftyImages asset.
 
- Select Send to Braze.
...
- Open an email template, campaign, or Canvas email message in Braze.
 
- In the message editor, open the personalization menu and select Content Blocks as the personalization type.
 
- Select the NiftyImages Content Block you published from NiftyImages.

### Reuse NiftyImages assets across Braze

- Use the published Content Block across marketing emails, email templates, campaigns, shared brand assets, and automated flows.
 
- When a NiftyImages asset uses dynamic variables, Braze passes the contact values based on the message and channel.
 
- Update the source asset in NiftyImages when you need creative changes.

### Disconnect a Braze account

- Return to Settings > Integrations in NiftyImages.
 
- Open the Braze connection page.
 
- Select the remove or disconnect icon for the account you want to remove.
...
Refer to the following table if you experience issues with the NiftyImages integration.
 ...
 Braze account doesn’t connect | 
 Confirm the REST API key is valid, the REST endpoint is correct, and the key includes the required permissions. | 

 Braze contact properties don’t appear in NiftyImages | 
 Confirm the API key includes custom_attributes.get. Then refresh the Braze connection inside NiftyImages. | 

 The asset doesn’t publish to Braze Content Blocks | 
 Confirm the API key includes content_blocks.create and that the connected Braze account allows Content Block creation. | 

 Personalization doesn’t show correctly | 
 Check that the selected Braze contact property contains a value for the test user. Add fallback values in NiftyImages where needed. | 

 The image doesn’t render in Braze | 
 Confirm the NiftyImages asset is saved, active, and published correctly. Send a Braze test message to verify the image in the intended channel. | 
