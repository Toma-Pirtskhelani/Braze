---
url: https://www.braze.com/docs/partners/additional_channels_and_extensions/additional_channels/direct_mail/mypostcard
slug: docs__partners__additional_channels_and_extensions__additional_channels__direct_mail__mypostcard
title: "MyPostcard"
...
## Prerequisites
 ...
 Registering with MyPostcard is required to take advantage of this integration. | 

 B2B API key and credentials | 
 You can find your API Key and the credentials in the MyPostcard B2B Admin Tool. | 

 Approved MyPostcard B2B campaign | 
 To take advantage of this integration, you need to set up a print mailing campaign in the MyPostcard B2B tool. | 
...
If you would like to create a one-off MyPostcard webhook campaign or use an existing template, select Webhook in Braze when creating a new campaign. Fill out the following fields:
 ...
 Webhook URL | 
 The webhook URL as shown in the B2B Admin Tool. | 

 Request Body | 
 Raw Text (JSON format found in the B2B Admin Tool). | 
