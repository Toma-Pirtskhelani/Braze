---
url: https://www.braze.com/docs/user_guide/channels/email/best_practices/email_guidelines
slug: docs__user_guide__channels__email__best_practices__email_guidelines
...
The recipient’s email client, not Braze, controls how alt text displays. For details on this behavior across clients like Gmail, Outlook, and Apple Mail, refer to How email clients display alt text.
...
Email addresses targeted through the Braze servers must be validated per RFC 2822 standards, Braze does not accept certain characters and recognizes them as invalid. If an email is bounced, Braze marks the email as invalid and the subscription status is not changed.

For information about disallowed characters and email validation rules, see Email validation.
...
You cannot forward replies sent to a sending subdomain into a personal inbox through Braze. To receive user replies, configure a separate Reply-To address on a domain you control with an inbound mailbox. See From and reply-to addresses.
...
- Check the user profile and review each Canvas and campaign the user received.
 
- Review the change logs to see if the campaign or Canvas was modified after launch. It’s possible the campaign or Canvas had the same subject line as the original when the user received it.
...
For more information on how Braze handles duplicate email addresses and deduplication, see the Email FAQ.
