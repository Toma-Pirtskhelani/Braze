---
url: https://www.braze.com/docs/user_guide/data/activation/events/custom_events/custom_event_properties
slug: docs__user_guide__data__activation__events__custom_events__custom_event_properties
...
Custom event properties aren’t stored on the Braze profile and therefore don’t log data points (see Data points for exceptions).
...
Property values must be an object: keys are the property names (non-empty strings, 255 characters or fewer, no leading $), and values are the property values. For supported data types, format requirements, and payload limits, see Data types.
...
You cannot use reserved keys as event property names. Using a reserved key in the properties object returns the error “Invalid ‘properties’ field”.
 ...
 Purchase events | 
 time, product_id, quantity, event_name, price, currency | 
...
The following message is personalized for three different segments using conditional logic. The custom event property called time_spent can be included in the message by calling {{event_properties.${time_spent}}}.
...
For a full list of Liquid tags that cause in-app messages to deliver as templated in-app messages, see Frequently asked questions.
...
- event_properties: Properties from a custom event or purchase that occurs during the journey. Use them only in the first Message step after an Action Paths step—not on the Everyone Else path, and not in later Message steps.
...
For more information, see Context and event properties and Canvas entry properties and event properties.
...
You need the “Edit Custom Event Property Segmentation” user permission to create segments based on event property recency and frequency.
...
- Go to your custom event and select Manage properties.
 
- Select the Enable segmentation toggle to add the event property for segmentation. You can access additional filtering options when segmenting.
...
- Within 30 days: You can use event property segmentation based on the frequency and recency of specific event property values within Braze segments. This option impacts data usage.

- Within and beyond 30 days: To cover both short-term and long-term event property segmentation, you can use Segment Extensions. This feature segments users based on custom events and event properties tracked within the past two years. This option does not impact data usage.
