---
url: https://www.braze.com/docs/user_guide/channels/whatsapp/whatsapp_setup/byo_connector
slug: docs__user_guide__channels__whatsapp__whatsapp_setup__byo_connector
title: "Bring Your Own WhatsApp connector"
description: "This reference article provides a step-by-step walkthrough for setting up a Bring Your Own WhatsApp connector, which gives Braze access to your Infobip WhatsApp Business..."
...
To migrate from other Business Solution Providers (BSPs) to the Braze integration, see Migrate from another Business Solution Provider.
...
## Requirements
 ...
 Infobip account | 
 An Infobip account is required to use the BYO WhatsApp connector. | 

 Message or Action Credits | 
 You consume Braze Action Credits when you send WhatsApp messages. | 

 WhatsApp requirements | 
 Complete all WhatsApp requirements. | 

 Phone number | 
 We suggest you acquire a phone number through Infobip for convenience. | 
...
- Follow the steps in Migrate between WhatsApp Business Accounts to migrate your phone numbers to a new WhatsApp Business Account one phone number at a time.
...
- In Infobip, identify the account that you want to use with your WhatsApp Business Account.
 
- Go to Developer Tools > API Keys and select Create API Key.

- Give the key a meaningful name, such as “Braze - My Workspace Name - My WABA Name”.
 
- Add an expiry date that is far into the future to avoid issues with token expiration.
...
- After creating the key, copy the API Key.
...
- Copy the account API base URL.

## Step 2: Start the embedded signup

- In Braze, go to Partner Integrations > Technology Partners > WhatsApp
 
- Select the BYO Connector - Infobip tab.

- Enter the API key and base URL from Step 1.
 
- Select Connect.
 
- Proceed through the Embedded Signup workflow with these considerations:
...
- Subscribing users to the subscription group
...
- Confirm your WhatsApp Business Account isn’t connected to Braze or another partner.
 
- Confirm no phone numbers in your WhatsApp Business Account are connected to a different Infobip account. For imported numbers, you can find the number in Infobip and select Cancel number.
...
While all existing functionality with Braze is supported, these use cases are currently not supported.
 ...
 Processing inbound messages in Braze and Infobip | 
 This prevents logic trains that are triggered by either system, consequently generating duplicate and potentially contradictory message threads. | 

 Sending messages from Braze and Infobip | 
 For WhatsApp Business Accounts connected to Braze, all sending originates from Braze. | 
