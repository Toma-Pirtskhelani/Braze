---
url: https://www.braze.com/docs/user_guide/brazeai/decisioning_studio/decisioning_studio_go/setup
slug: docs__user_guide__brazeai__decisioning_studio__decisioning_studio_go__setup
title: "Set up your Decisioning Studio Go agent"
...
For an overview of how Decisioning Studio Go works, see BrazeAI Decisioning Studio Go. To confirm your program is a good fit before you configure an agent, see Examples for Decisioning Studio Go.

## Prerequisites

Confirm you have the following ready:

- A segment for your entry audience that isn’t actively used in another Canvas or campaign
...
- In the Braze dashboard, go to the AI Decisioning tab.
 
- Select Create Agent.
 
- Give your agent a name that distinguishes this agent from others in your workspace. An example is “Loyalty Members—Weekly Engagement” rather than “Email Agent”.
 
- (Optional) Add a description to provide context that you or a teammate may need later. This can include what the agent is for, which segment it targets, and what success looks like.
...
Audience filters aren’t supported in this release. If you need additional targeting criteria, create a segment with those filters applied, then select that segment as your entry audience.

### Integrate with existing Canvases

To use Decisioning Studio Go inside a broader journey:

- Create a dedicated segment for users who should be in the agent.
 
- In your Canvas, use a User Update step to add the user to that segment at the right point in the journey.
 
- Confirm that users exit the Canvas so the agent (not the Canvas) handles email sending for everyone in the segment from that point on.
...
Content blocks aren’t supported as substitution points for personalized components. Place your personalized subject line, CTA, and image directly in the email body rather than inside a content block.
...
The reporting interface offers three main views:
 ...
 Performance | 
 Click rates, engagement metrics, and the lift of the Decisioning Studio group versus the random control. | 

 Configuration | 
 The current settings of the agent—useful for confirming what’s running. | 

 Agent preferences | 
 Counts of how often each variant has been chosen by the agent, showing what the agent is gravitating toward for your audience. | 
...
The following table summarizes areas of Decisioning Studio Go and related details.
 ...
 Conversion metric | 
 Clicks only (unique daily clicks per user) | 

 Personalization points | 
 Subject line, CTA, hero image (per base creative) | 

 Audience | 
 One Braze segment, with required random control (minimum 5%) | 

 Frequency | 
 Single selection (no frequency decisioning) | 

 Test sends | 
 Through Braze composer | 

 Reporting | 
 Performance, Configuration, and Agent preferences views, plus element-level breakdowns | 
...
For common questions, see the Decisioning Studio Go FAQ.
