---
url: https://www.braze.com/docs/user_guide/administer/global/user_management/internal_groups
slug: docs__user_guide__administer__global__user_management__internal_groups
...
To create and manage internal groups, you need the following user permissions:
...
- Choose one or more group types, as listed in the following table.
 ...
 User Event Group | 
 Use this to verify events or logs from your test device.

To capture SDK and REST API logs for group members, select the User Events checkbox. Without this setting, users added to the group won’t show logs in the Event User Log. | 

 Content Test Group | 
 Use this across push, email, and in-app messages to send a rendered copy of the message. | 

 Seed Group | 
 Automatically sends a copy of the email to everyone in the Seed Group upon send. | 

- Select Create internal group again.
...
- Choose from the following methods for searching and selecting your test users.
 ...
 Add identified user | 
 Search for the user by their external user ID, email address, phone number, or push token. | 

 Add anonymous user | 
 Search by IP address. Then, provide a name for each test user you add. This is the name that all event logs are associated with on the Event User Log page. | 

 Bulk add users | 
 Copy and paste a list of email addresses or external IDs. You can add only users who are already known in the dashboard. For more information, refer to User import. | 
...
If the email uses abort_message() Liquid, Seed Group members must still satisfy the abort condition to receive the send.
