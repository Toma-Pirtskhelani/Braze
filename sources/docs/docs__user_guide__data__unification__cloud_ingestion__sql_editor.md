---
url: https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion/sql_editor
slug: docs__user_guide__data__unification__cloud_ingestion__sql_editor
title: "Cloud Data Ingestion: SQL Editor"
...
Cloud Data Ingestion’s SQL Editor lets you create syncs by writing SQL queries directly against your data warehouse. This removes the need to create or maintain a dedicated CDI table, which was previously required in Step 1.1 of Data Warehouse Integrations.
...
Note that these steps use a Snowflake source as an example. The setup process for other data warehouse sources is similar and can be found in Step 2: Create a new source in Braze dashboard of the Setting up data warehouse integrations documentation.
...
- In the Braze dashboard, go to Data Settings > Cloud Data Ingestion > Sources.
...
For the Snowflake Account Locator field, enter your Snowflake account identifier, which typically follows a format like xy12345.us-east-1.aws. This is not the same as a database name or warehouse name.
...
For additional information, see Snowflake key-pair authentication. If you want to rotate keys at any point, Braze can generate a new key pair and provide the new public key.
...
- Go to Data Settings > Cloud Data Ingestion > Syncs.
 
- Select Create data sync.
 
- Choose any sync under Data Type.
 
- Reference the source from Step 2.
...
Only read-only queries are supported, including JOIN clauses. For more details, see SQL constraints.
...
To successfully validate, your SQL query must return various required columns:
 ...
 Attributes | 
 - A user identifier, one of external_id, braze_id, alias_name and alias_label, email or phone number.
- UPDATED_AT.
- At least one additional column (attribute) to sync. | 
...
- UPDATED_AT. | 
 ...
 - A user identifier, one of external_id, braze_id, alias_name and alias_label, email or phone number.
- UPDATED_AT.
- NAME to represent the event name.
- TIME to represent the event time. If unavailable, CDI uses UPDATED_AT as a substitute. | 
...
- CURRENCY.
- PRICE.
- TIME to represent the purchase event time. If unavailable, CDI uses UPDATED_AT as a substitute. | 

 Catalog | 
 - ID to represent the catalog item identifier.
- UPDATED_AT.
- At least one additional column (catalog field) to sync. | 

 Accounts | 
 - ID to represent the account identifier.
- NAME to represent the account name.
- UPDATED_AT.
- At least one additional column (account field) to sync. | 

Additional columns outside of required columns are synced as attributes, Canvas context properties, event properties, catalog fields, and account fields, respectively. See Validation behavior and Troubleshooting for helpful tips about preview and validate errors and how to fix them.
...
In this case, the preview still appears to help you move toward a successful validation. See Step 4 in the previous section for details on required columns for each sync data type.
...
SQL Editor supports legacy CDI tables where a PAYLOAD column is present.
