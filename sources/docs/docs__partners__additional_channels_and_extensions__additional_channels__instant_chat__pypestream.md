---
url: https://www.braze.com/docs/partners/additional_channels_and_extensions/additional_channels/instant_chat/pypestream
slug: docs__partners__additional_channels_and_extensions__additional_channels__instant_chat__pypestream
title: "Pypestream"
...
## Prerequisites
 ...
 Pypestream account | 
 A Pypestream account is required to take advantage of this partnership.

Once subscribed, the Pypestream team will help you set up your dedicated environment to begin building your conversational AI solution to integrate with Braze. | 
...
Visit this Pypestream article for an overview and configuration steps for Pypestream action nodes. You must be a Pypestream customer to access this documentation.
...
The final step of integrating with the Braze REST API involves configuring the flows within Pypestream’s Design Studio to use the action node that was developed in the previous step.

note

Visit this Pypestream article for an overview on how to configure modes within Design Studio. You must be a Pypestream customer to access this documentation.

## Integration use case

After the prerequisites are met, and an action node structure has been created, the developer has a blank Canvas to work from when interacting with the Braze API endpoints. This example shows the steps required to integrate an action node into the Braze /user/track endpoint-specifically to create a user profile to track a specific user entering a Pypestream conversational flow.
...
This data can now be sent to the Braze platform to track this user’s engagement with the ability to potentially retarget them in the future. Check out the use case list to view common applications.
