---
url: https://www.braze.com/docs/user_guide/channels/push/push_setup/push_token_lifecycle
slug: docs__user_guide__channels__push__push_setup__push_token_lifecycle
title: "Push token lifecycle"
description: "This reference article discusses what it means to be registered for push and how we send push messages and deal with push tokens and push..."
...
Keep in mind, push tokens aren’t static identifiers that last forever—they can be updated and they can expire.

tip

For platform-specific details, see Push token registration.
...
Push tokens are used to send both foreground and background push notifications.
 ...
 Foreground push | 
 Yes | 
 A notification is visibly displayed to the user while the app is in the foreground. | 

 Background push | 
 No | 
 A notification is silently delivered in the background without being displayed. Often used for functionality like uninstall tracking. | 
...
This is different from the Foreground Push Enabled segmentation filter, which is used to identify users who have opted-in to at least one of your apps—not one specific app. For more information, see Segmentation filters.
...
If you see an error message in Contact Settings > Push Changelog, refer to Common push error messages for explanations and next steps.
...
When your app is installed, a push token is automatically generated for your app—however, it can only be used for background push notifications until the user explicitly opts-in. Additionally, registration is handled differently across different Android versions:

 Version | 
 Details | 

 Android 13 | 
 Push permission must be requested and granted by the user. Your app can request permission manually, or users will be prompted automatically after a notification channel is created. | 

 Android 12 and earlier | 
 All users are considered Subscribed after their first session. Braze automatically requests a push token at this point, making the user push enabled with a valid token and a default subscription state of Subscribed. | 
...
iOS does not automatically generate push tokens for an app when it’s installed. Additionally, registration is handled differently across different iOS versions:
 ...
 iOS 12 | 
 Yes | 
 When a user opts-in to push notifications, you’re given standard authorization, allowing you to send foreground push notifications. However, you can also request provisional authorization, which let’s you send silent background push notifications directly to the notification center. | 

 iOS 11 or earlier | 
 No | 
 All users must explicitly opt-in to receive push notifications. A push token is generated only after permission is granted. | 

### Checking user’s push subscription state

There are two ways you can check a user’s push subscription state with Braze:

- User Profile: You can access individual user profiles through the Braze dashboard on the User Search page. After finding a user’s profile (by email address, phone number, or external user ID), you can select the Engagement tab to view and manually adjust a user’s subscription state.
...
On the other hand, if the device entry’s app name is prefixed by Background:, the app is only authorized to receive background push and cannot display user-visible notifications on that device. This usually indicates the user has disabled notifications for the app on that device.
