---
url: https://www.braze.com/docs/partners/message_orchestration/cms_dam/bynder
slug: docs__partners__message_orchestration__cms_dam__bynder
title: "Bynder"
description: "This reference article outlines the partnership between Braze and Bynder, a digital asset management (DAM) platform that lets you search for and insert approved asset..."
...
This integration is maintained by Bynder.
...
Before you start, you need the following:
 ...
 A Bynder account | 
 A Bynder account with access to the DAM assets you want to reference in Braze. | 

 Bynder Universal Compact View (UCV) Chrome extension | 
 Installed from the Chrome Web Store and connected to your Bynder portal. Available for Google Chrome only. | 

 Public assets and derivatives | 
 Any asset, and the specific derivative you plan to link to, must be marked public in Bynder so its URL resolves correctly for message recipients. | 

 A Braze account | 
 Access to the messaging channel (email, Content Block, in-app message, Canvas, and so on) where the asset is used. | 

## Integration

### Step 1: Install and connect the Bynder UCV Chrome extension

- Go to the Bynder UCV extension in the Chrome Web Store.
 
- Click Add to Chrome, review the requested permissions, then click Add extension.
 
- From the Chrome toolbar, select the Bynder UCV icon (pin it to the toolbar first if it isn’t already visible).
 
- Enter your Bynder portal domain (without https://), then click Connect.
 
- In the window that opens, log in to your Bynder portal with your usual credentials.

### Step 2: Search for and select a Bynder asset

- With the extension connected, open the Universal Compact View from any browser tab, including your Braze dashboard.
 
- Use the smart filter and search bar to locate the image, video, document, or audio asset you need.
 
- Select the asset, then select the derivative (or the original file, if it’s public) you want to use.
 
- Click Add Asset to copy the selected asset’s URL to your clipboard.

### Step 3: Add the asset URL to your Braze campaign

- In Braze, open the email, Content Block, in-app message, or Canvas step where you want to add the asset.
 
- 
 
Paste the copied Bynder asset URL into the relevant field. For example, use an <img src=""> tag or a Content Block’s image URL field.
...
- Save and preview your message to confirm the asset renders as expected.
...
## Troubleshooting
 ...
 Pin the Bynder UCV extension to the Chrome toolbar from the Extensions menu. | 

 Connect doesn’t open a login window | 
 Confirm Chrome pop-ups are allowed for your Bynder portal domain, and close any other tab where the portal is already open. | 

 Asset URL doesn’t render in Braze | 
 Confirm that the asset and the specific derivative used are marked public in the Bynder portal. | 

For more information, see Bynder’s Universal Compact View documentation or contact Bynder Support.
