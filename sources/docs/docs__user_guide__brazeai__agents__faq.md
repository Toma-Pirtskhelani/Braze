---
url: https://www.braze.com/docs/user_guide/brazeai/agents/faq
slug: docs__user_guide__brazeai__agents__faq
title: "Agents frequently asked questions"
...
Go to Settings > Billing > Credits Usage > Agent Console to see credit consumption, invocation counts, and per-agent credit ratios. See Daily invocation and credit limits for more details.
...
No. Agents only receive the specific user data points that are passed using Liquid in instructions, + Agent context selections, upstream Context steps in Canvas, or additional context on the Agent step. Agents cannot search user profiles for attributes you did not configure them to receive.

Agents also cannot warn you when required data is missing—they proceed with whatever is in the prompt. Treat agent setup as deliberate input-to-output design: pass every field the agent needs and verify inputs in Agent Console > Logs. For guidance, see What data agents receive.

## Troubleshooting

### Why did my agent not follow my instructions or rules?

Consider using Operator to troubleshoot why your agent is not following your instructions. Operator can provide step-by-step instructions and detailed explanations.
...
For more details, see Catalog Agent best practices.
...
It depends on the failure type:
 ...
 Daily invocation limit reached | 
 No | 

 Timeout | 
 Yes | 

See When credits are consumed for details.

### My agent is struggling with a complex task. How might I improve its performance?
...
- The agent’s task is too complex and would benefit from a sub-agent approach instead
...
- Choose a simpler or lower-cost model: Faster models usually complete within the default timeout window. See Determine which model to use.
 
- Lower the thinking level (BYO models only): Start at Minimal and increase only if output quality suffers. See Thinking levels.
 
- Simplify the prompt: Remove redundant instructions, shorten examples, and narrow the output schema. Use Operator to review and tighten your instructions.
 
- Split complex workflows into multiple agents: If the use case has several sub-steps (for example, classify intent, then generate copy), use separate agents in sequence in Canvas or catalog instead of one agent that does everything. See sub-agent approach.

note

Timeouts consume Braze credits even when the agent returns no usable output. See When credits are consumed.

For Canvas Step Agents, configure fallback values in Agent Console so users still receive output when an invocation fails.
