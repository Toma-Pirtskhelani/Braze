---
url: https://www.braze.com/docs/partners/data_and_analytics/data_warehouses/snowflake/sample_queries
slug: docs__partners__data_and_analytics__data_warehouses__snowflake__sample_queries
...
- Partition the events by the key (app_group_id, message_variation_id, dispatch_id, email_address).
 
- In each partition, order the events by time, and the first event is always a unique event.
 
- For every subsequent event, if it occurred more than seven days after its predecessor, is considered a unique event.
...
- In each partition, order the events by time. The first event is always a unique event.
 
- For every subsequent event, if it occurred more than seven days after its predecessor, it’s considered a unique event.

You can use Snowflake’s windowing functions to achieve this. The following query returns all email opens in the last 365 days and indicates which events are unique in the is_unique column:
