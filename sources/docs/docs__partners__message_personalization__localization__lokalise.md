---
url: https://www.braze.com/docs/partners/message_personalization/localization/lokalise
slug: docs__partners__message_personalization__localization__lokalise
title: "Lokalise"
...
### Prerequisites
 ...
 Lokalise account | 
 A Lokalise account is required to take advantage of this partnership. | 

 Lokalise translation project | 
 Create a Lokalise project with the Marketing and support type and choose Braze as the Content integration. | 

 Braze multi-language settings | 
 Multi-language support must be enabled in your Braze workspace. | 

 Braze REST API key | 
 A Braze REST API key with permissions to read and update Campaigns, Canvases, and Email Templates. You can create one in the Braze dashboard from Settings > API Keys. | 

 Braze server region | 
 Your Braze server region (for example, US-01, EU-01). You can find this in the Braze dashboard. | 

 Translation tags in Braze content | 
 Messages must use translation tags to identify translatable content. Wrap each translatable block in {% translation ID %}...{% endtranslation %} tags with a unique ID. | 

### Setup and usage

For detailed instructions on connecting the Braze Multi-language integration in Lokalise, importing content, translating, and exporting translations back to Braze, refer to Lokalise’s Braze integration documentation.
...
### Prerequisites
 ...
 Lokalise translation project | 
 Create a Lokalise project with the Software Localization project type. | 
...
- Open your Lokalise project, click Add Key, enter “description” in the Key field.
...
- The JSON file name at the end of the URL is based on the language code.
