---
url: https://www.braze.com/docs/user_guide/example_library/catalogs/match_catalog_items_to_attribute_array
slug: docs__user_guide__example_library__catalogs__match_catalog_items_to_attribute_array
title: "Match catalog items to a custom attribute array"
...
- Assigns the user’s array custom attribute to a Liquid variable.
 
- Calls catalog_selection_items for a pre-filtered catalog selection (up to 50 items).
 
- Loops over items and uses contains to match each catalog field (for example, name or id) against the array.
...
- For an array of objects, match on a property inside each object (for example, product_id) and adjust the contains check or use a for loop over objects. See Array of objects.
 
- contains behavior depends on attribute type; for arrays, use contains rather than ==. See Conditional logic.
...
This example assumes:
 ...
 Custom attribute | 
 saved_product_names — string array (for example, ["linen_shirt", "trail_jacket", "canvas_tote"]) | 

 Catalog | 
 apparel_products with columns id, category, name, price, inventory, image_url | 

 Selection | 
 in_stock_apparel on apparel_products, results limit 50, with filters that exclude irrelevant rows (for example, inventory greater than 0) | 

### Step 1: Create the catalog and selection

- Import or sync product rows into a catalog named apparel_products.
 
- Create a selection (for example, in_stock_apparel) that returns as many relevant rows as you need, up to the 50 item limit.
 
- Add selection filters to drop rows you never want in the message (out of stock, wrong category, and so on).

For selection setup, see Selections.
...
To generate this Liquid automatically, open the Add Personalization modal (Catalog Items > Use a selection). See Using catalogs.
...
- Liquid use case library — find a string within an array
