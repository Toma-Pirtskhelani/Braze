---
url: https://www.braze.com/docs/developer_guide/rest_api/sending_sms_messages
slug: docs__developer_guide__rest_api__sending_sms_messages
title: "Sending SMS messages using the REST API"
...
- Optionally, switch to API-triggered delivery to define your message templates in the Braze dashboard while still triggering sends from your backend.

To send an SMS message through the REST API, you need to set up an API campaign in the Braze dashboard, then use the /messages/send endpoint to send the message.
...
To complete this guide, you need:
 ...
 SMS subscription group | 
 An SMS subscription group configured in your Braze workspace. | 
...
- Select Create Campaign, then select API Campaigns.
 
- Enter a name and description for your campaign, such as “SMS message notification”.
 
- Add relevant tags for identification and tracking.
 
- Select Add Messaging Channel, then select SMS.
 
- Note the Campaign ID and Message Variation ID displayed on the campaign page. You’ll need both values when constructing your API request.

## Step 2: Send an SMS message using the API

Construct a POST request to the /messages/send endpoint. Include the campaign ID, the recipient’s external user ID, and the SMS content in the request payload.
...
Replace the placeholder values with your actual IDs. The body field supports Liquid personalization, so you can tailor the message content to each recipient. For the full list of parameters supported by the SMS messaging object, see SMS object.
...
- Confirm the SMS message is delivered to your phone.
...
- Confirm that your SMS campaigns comply with relevant regulations and carrier requirements. Include opt-out instructions (such as “Text STOP to opt out”) in every message. For more information, see SMS laws and regulations and Opt-in and opt-out keywords.
 
- Use Braze personalization features to tailor SMS content to individual consumers, including dynamic content and user-specific data.
