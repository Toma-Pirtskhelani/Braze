---
url: https://www.braze.com/docs/partners/ecommerce/ordering_payments_subscription/recurly
slug: docs__partners__ecommerce__ordering_payments_subscription__recurly
title: "Recurly"
...
Additional details around using Recurly with Braze can be found in the Recurly Docs.
...
## Prerequisites
 ...
 Recurly account | 
 An Elite Recurly subscription plan with the Braze feature flag enabled is required to take advantage of this partnership. The activation of credit invoices in your Recurly platform is also required. | 

 Braze REST API key | 
 A Braze REST API key with users.track permissions. 

 This can be created in the Braze dashboard from Settings > API Keys. Because Recurly only uses the users.track endpoint, we recommend provisioning a Recurly specific key with only this permission. | 
...
For effective customer engagement, you must configure custom events in Braze to receive events triggered by Recurly. Ensure to include each event from Recurly for thorough data integration. These events can also be tracked within Braze analytics. Once configured, these custom events can be used to segment users or personalize messaging.
 ...
 Triggered when a subscription’s attributes change (plan change, price change, or quantity change) | 
...
Batches are sent to Braze in groups of up to 75 events at a time. For example, if 100 subscriptions were created at once, Recurly would make two API requests to Braze. See batching User Track requests for details.
