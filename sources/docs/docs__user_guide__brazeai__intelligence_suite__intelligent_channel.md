---
url: https://www.braze.com/docs/user_guide/brazeai/intelligence_suite/intelligent_channel
slug: docs__user_guide__brazeai__intelligence_suite__intelligent_channel
title: "Intelligent Channel filter"
description: "This article covers the Intelligent Channel filter, a filter that selects the portion of your audience for whom the selected messaging channel is their best..."
...
- Email: Opens (machine opens excluded). Email clicks are not included.
...
Push influenced opens, email clicks, and session activity are not used for Intelligent Channel. Session activity is used by Intelligent Timing.

Intelligent Channel does not support webhooks, LINE, Kakao Talk, in-app messages, or Content Cards.

important

To compute the engagement rate of the SMS channel, turn on SMS link shortening with advanced tracking and click tracking. Without this tracking, SMS may be selected as the Intelligent Channel for a 0% engagement rate because of our tie-breaking behavior.
...
Campaigns and Canvas steps that ignore frequency capping are not accounted for by Intelligent Channel and cannot contribute to the data requirements.
...
## Message Open Likelihood filter for individual channels

Rather than let Braze choose the single best channel for a user, you can use the “Message Open Likelihood” segmentation filter to filter users based on whether or not they’re likely to open a message on a specific channel you choose. This filter is calculated by the percentage of interactions divided by the total messages received for the last 100 messages sent per channel.
