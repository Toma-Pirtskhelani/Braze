---
url: https://www.braze.com/docs/user_guide/messaging/templates/canvas_templates/braze_templates/back_in_stock
slug: docs__user_guide__messaging__templates__canvas_templates__braze_templates__back_in_stock
title: "Back in stock"
description: "This article describes how to use a Braze Canvas template to drive purchases by notifying your users when an item is back in stock with..."
...
- A catalog containing information about your item
 
- Back-in-stock notifications must be set up for the item you want to message users about
...
Before creating the Canvas, we set up a catalog that contains information about our straight leg pants inventory and set up back-in-stock notifications for the Classic Straight Leg jeans. We made it so that users will subscribe to notifications after they perform the custom event of favoriting the Classic Straight Leg jeans on the app.
...
- Update the Canvas name to specify that the Canvas is for targeting users when our product Classic Straight Leg is back in stock.
 
- Update the description to explain that this Canvas contains personalized messaging.
 
- Add the tag Back in Stock, which is nested under the tag Promotional, so that we can filter for it on the Canvas home page.
...
- Select the catalog that includes information about our Classic Straight Leg jeans, which we’ve named “Straight Leg Pants”.

- Set the Start Time (Required) to our desired start date and time.
...
- Select our target segment, “Favorited - Classic Straight Leg Jeans”, which consists of users who’ve favorited our Classic Straight Leg jeans on our app or website.
 
- Select a filter to include users who have purchased “Jeans” more than “0” times.

- Adjust the entry controls to allow users to re-enter the Canvas after the Canvas’s maximum duration, to prevent the likelihood of users triggering the same step concurrently.

- Adjust the exit criteria to remove users who performed the custom event of unfavoriting the Classic Straight Leg jeans.
...
Now, we’ll build our Canvas by customizing the channels and content that will send to users. Because we’re using all four of the template channels (mobile and web push, SMS, and email) and using the Intelligent Channel filter, we don’t need to add or remove any.
...
- Replace !!YOURCATALOGHERE!! with our catalog name (“Straight_Leg_Pants”).
 
- Replace [0] with the index number of the Classic Straight Leg jeans, which is “9” because the jeans are tenth item in the items array of our catalog. (Arrays are zero-indexed in Liquid, so the first item is 0 and not 1.)
 
- Repeat steps 1 and 2 for all remaining Message steps, including:
...
- Update the Action Paths step by selecting the Purchase action group. Then, select Make a specific purchase and choose Classic Straight Leg jeans for the product.
