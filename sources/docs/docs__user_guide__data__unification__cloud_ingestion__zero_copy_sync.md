---
url: https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion/zero_copy_sync
slug: docs__user_guide__data__unification__cloud_ingestion__zero_copy_sync
title: "Zero-copy personalization using CDI"
...
If you’re already familiar with Braze CDI, note that the setup for a Canvas trigger sync closely follows the process for user-data CDI integrations, with the following caveats:
...
If your account has network policies, allowlist the Braze IPs to enable the CDI service connection. For the list of IPs, see Cloud Data Ingestion.
...
Create a user and grant permissions. If you already have credentials from another sync, you can reuse them as long as they have access to the canvas triggers table.
 ...
 Allows Braze to connect. | 

 BigQuery User | 
 Allows Braze to run queries, read metadata, and list tables. | 

 BigQuery Data Viewer | 
 Allows Braze to view datasets and contents. | 

 BigQuery Job User | 
 Allows Braze to run jobs. | 

After granting permissions, generate a JSON key. See Keys create and delete for instructions. You’ll upload it in the Braze dashboard later.
...
To sync Canvas triggers from file storage, create a source file with the following fields.
 ...
 EXTERNAL_ID | 
 Yes, one of external_id or alias_name, and alias_label | 
 ...
 PROPERTIES | 
 Yes | 
 JSON string of fields to make available as personalization properties in your Canvas. This should contain user-specific information. | 
...
- Set up your destination Canvas for Canvas triggers. Create a new or select an existing API-triggered Canvas. Refer to Entry schedule types for instructions on how to create a canvas with an API-triggered delivery schedule type.
...
- Within your Canvas steps, use Canvas entry properties to personalize messages with properties fields that you plan to sync from your source table.
...
- After building your Canvas, launch it and proceed to Step 3.

#### Step 3: Create your zero copy sync

With your source setup complete and destination Canvas launched, create a new data sync:

- In Braze, go to Data Settings > Cloud Data Ingestion .
 
- Set up the connection by entering connection details (or reuse existing credentials) and the source table from Step 1.
 
- Provide a name the integration.
 
- Select the Canvas triggers data type.
...
- Select Test Connection to confirm everything works as expected. If connecting to Snowflake, first add the public key displayed on the dashboard to the user created for Braze to connect to Snowflake. To complete this step, you’ll need SECURITYADMIN access or higher in Snowflake.
...
- Using CDI Canvas triggers when already saturating your REST API’s rate limit for /canvas/trigger/send.
