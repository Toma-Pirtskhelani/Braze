---
url: https://www.braze.com/docs/user_guide/messaging/design_and_edit/personalize/connected_content/debugger
slug: docs__user_guide__messaging__design_and_edit__personalize__connected_content__debugger
title: "Connected Content Debugger"
...
- Configure your message with the {% connected_content %} tag.
 
- Go to the Preview & Test section. If your message includes a Connected Content tag, you can see a summary view with the number of Connected Content calls and the successful and error statuses.

- Select View details to open the debugger alongside your preview. The drawer displays a table of the URL and outcome for each Connected Content call.

- Next to each URL and outcome, select View to view the request and response headers, payload, method, duration, and caching information.

- Review the results, adjust your tag, headers, or endpoint as needed. Then, generate a new preview to confirm the fix.
...
### URL details
 ...
 URL | 
 The fully rendered URL Braze called, with any Liquid tags resolved. | 

 Method | 
 The HTTP method used (GET or POST). | 

 Status code | 
 The HTTP status code your endpoint returned (for example, 200, 404, 500). See Troubleshooting response codes for Braze-specific codes. | 
...
### Response tab
 ...
 Duration | 
 How long the request took to complete, in seconds. Duration is shown only for live (non-cached) calls. | 

 Served from cache | 
 Indicates whether this response was served from Braze’s Connected Content cache rather than a live call to your endpoint (Yes or No). A cached result reflects an earlier response, not necessarily your endpoint’s current state. | 

 Response body | 
 The body returned by your endpoint. | 
...
### Request tab
 ...
 Headers | 
 Headers from your Connected Content tag (:headers, credentials, and options such as :content_type). | 

 Body | 
 The request body sent, if any (POST requests). | 
...
If your Connected Content tag uses :basic_auth, common secret headers, keys, or other auth credential options, the debugger redacts those values in the Request tab and replaces them with a series of asterisks (*). This lets you confirm that credentials were included in the request without exposing the values in Preview & Test.
...
If you see response codes such as 408, 429, 502, 503, 504, or 599, the issue is typically on Braze’s side of the call — related to host health, timeout, or payload size. If your endpoint consistently returns large responses, consider trimming the response payload to only the fields your message needs.
