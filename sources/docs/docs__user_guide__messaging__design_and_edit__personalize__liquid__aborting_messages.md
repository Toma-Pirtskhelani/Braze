---
url: https://www.braze.com/docs/user_guide/messaging/design_and_edit/personalize/liquid/aborting_messages
slug: docs__user_guide__messaging__design_and_edit__personalize__liquid__aborting_messages
title: "Abort messages"
...
- In the Braze dashboard, open the Message Activity Log for the campaign or Canvas message step.
 
- Filter for abort-related entries. By default, Braze logs {% abort_message %} called. If you passed a reason string to abort_message(), that text appears instead.
 
- Note whether aborts cluster on one channel (for example email only) or across channels in the same Canvas.
