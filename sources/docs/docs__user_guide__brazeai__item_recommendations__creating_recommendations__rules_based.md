---
url: https://www.braze.com/docs/user_guide/brazeai/item_recommendations/creating_recommendations/rules_based
slug: docs__user_guide__brazeai__item_recommendations__creating_recommendations__rules_based
title: "Create rules-based item recommendations"
...
When deciding which recommendation engine suits your available resources and use cases, refer to this table of considerations:
 ...
 No-code solution | 
 No advanced Liquid | 
 Automatically updates product feed | 
 Generated with Braze UI | 
 No data hosting or troubleshooting | 

 Catalogs CSV | 
 ✔ | 
 Yes, if using pre-generated Liquid. | 
 ✔ | 
 Yes, if recommendations are not updated frequently. | 
 ✔ | 
 ✔ | 
 ...
 | 
 ✔ | 
 Yes, if recommendations are updated hourly. | 
 ✔ | 
 ✔ | 
...
- For each product, add a list of recommended products as a string separated by a delimiter (like a pipe |) in a column named “product_recommendations”.
 
- Pass the product ID that you want to find recommendations for to the catalog.
 
- Get the product_recommendations value for that catalog item and split it by the delimiter with a Liquid split filter.
 
- Pass one or more of those IDs back to the catalog to collect the other product details.
...
Let’s say you have a health food app and want to create a Content Card campaign that sends different recipes based on how long a user has been signed up for your app. First, create and upload a catalog through an CSV file that includes the following information:
 ...
 id | 
 A unique number that correlates to the number of days since the user signed up with your app. For example, 3 correlates to three days. | 

 type | 
 The recipe category, such as comfort, fresh, and others. | 

 title | 
 The title of the content card that will be sent for each ID, such as “Make ahead for lunch this week” or “Let’s taco about it”. | 

 link | 
 The link to the recipe article. | 
...
To create your recommendation engine using Connected Content, first create a new endpoint using one of the following methods:
 ...
 Use a third-party recommendation engine, such as one of our Alloy partners, including Amazon Personalise, Certona, Dynamic Yield, and others. | 
...
- Use assign to create blank arrays for rating categories of “excellent”, “very good”, and “good”.
 
- Add a for loop that examines the rating of each restaurant in the list.
