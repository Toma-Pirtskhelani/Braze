---
url: https://www.braze.com/docs/user_guide/messaging/canvas/testing_canvases/measuring_and_testing_with_canvas_analytics
slug: docs__user_guide__messaging__canvas__testing_canvases__measuring_and_testing_with_canvas_analytics
...
This is a great place to get a high-level overview to check how your Canvas is performing against your goal. To get notified proactively if a Canvas’s performance drops outside your expected range, refer to Canvas threshold alerts.

### Reachable users and exact statistics

When Calculate exact statistics is running for audiences tied to your Canvas, Braze may briefly show a rounded estimate in the Reachable users area. The exact total replaces the estimate when the calculation finishes. Select Show Additional Stats for a full per-channel breakdown. The Canvas builder documents the same flow under Target Population; see Calculating target population.
...
- Total Revenue: The total revenue in dollars from Canvas recipients within the set primary conversion window. Total Revenue is the sum of purchases attributed to users who received that variant during that window. Purchases still count toward Total Revenue even when the user does not perform the configured primary conversion event, as long as the purchase falls within the attribution rules for the window.

note

Like conversions, revenue is technically tracked at the Canvas level, but is attributed to the most recent component and most recent variant from which the user has received a message (or entered, if they haven’t received a message yet).
...
Beyond that, you can see a more explicit breakdown of conversion events, including the following:
...
- User A enters a Canvas.
 
- The first step is a Delay step within the set Quiet Hours, so the message is suppressed.
 
- User A performs the conversion event.
...
User retention is one of the most important metrics for any marketer. Keeping engaged users coming back for more indicates that business is healthy. Braze now allows you to measure user retention right on the Canvas Analytics page. For more information on how to read and interpret your retention report, check out Retention reports.
