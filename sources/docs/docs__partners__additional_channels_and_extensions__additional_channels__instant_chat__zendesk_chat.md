---
url: https://www.braze.com/docs/partners/additional_channels_and_extensions/additional_channels/instant_chat/zendesk_chat
slug: docs__partners__additional_channels_and_extensions__additional_channels__instant_chat__zendesk_chat
title: "Zendesk Chat"
...
Zendesk Chat uses webhooks from each platform to set up a two-way SMS conversation. When a user requests support, a ticket is created in Zendesk. Agent responses are forwarded to Braze through an API-triggered SMS campaign, and user replies are sent back to Zendesk.
...
## Prerequisites
 ...
 A Zendesk account | 
 A Zendesk account is required to take advantage of this partnership. | 

 A Zendesk Basic Authorization Token | 
 A Zendesk Basic Authorization Token is used to make an outbound webhook request from Braze to Zendesk. | 

 A Braze REST API Key | 
 A Braze REST API key with campaigns.trigger.send permissions. This can be created in the Braze dashboard from Settings > API Keys. | 
...
- In the Zendesk developer console, go to webhooks: https://{{url}}.zendesk.com/admin/apps-integrations/webhooks/webhooks
 
- Under Create Webhook, select Trigger or automation.
 
- For Endpoint URL, add the /campaign/trigger/send endpoint.
 
- Under Authentication, select Bearer token and add the Braze REST API key with campaigns.trigger.send permissions.
...
- Create a new category (for example, Trigger a message).
 
- Create a new trigger (for example, Respond via SMS Braze).
...
Next, you’ll create two new webhook campaigns in Braze so you can forward inbound SMS from customers to the Zendesk inbox.
 ...
 Webhook campaign 2 | 
 Forwards all conversational SMS responses sent inbound from the customer to Zendesk. | 
...
In the Braze dashboard, go to Audience, choose your SMS subscription group, then select Add Custom Keyword. Fill out the following fields to create an exclusive SMS keyword category for Zendesk.
 ...
 Keyword Category | 
 The name of your keyword category, such as ZendeskSMS1. | 

 Keywords | 
 Your custom keywords, such as SUPPORT. | 

 Reply Message | 
 The message that is sent when a keyword is detected, such as “A customer service rep will contact you shortly.” | 
