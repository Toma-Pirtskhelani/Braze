---
url: https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion/visual_mapper
slug: docs__user_guide__data__unification__cloud_ingestion__visual_mapper
title: "Cloud Data Ingestion: Visual mapper"
...
- An active Cloud Data Ingestion data warehouse source. If you haven’t set one up, see Data warehouse integrations.
...
- Go to Data Settings > Cloud Data Ingestion > Syncs, then select Create data sync.
 
- Choose a name for your sync and select an active data source. Only active sources can be used.
 
- For Data destination, select Braze Data Platform.
 
- For Data Type, select User Attributes.
 
- Select Next: Data definition.

### Step 2: Map your source schema

- On the Data definition step, select Visual mapper.
 
- In the Table field, enter the table or view name as it appears in your data warehouse.
 
- Select Map source schema. Braze reads the schema of your table or view and lists every column with its detected data type.
...
- On the Notifications step, enter at least one contact email for sync error notifications. You can optionally turn on Row Error alerts (sent when a percentage of rows fail to update) and Sync success notifications.
 
- On the Schedule step, turn on Recurring sync to run the sync on a schedule, or leave it off for a one-time sync.
 
- Review the Summary step. It lists your configuration, which attributes are new versus existing, and any columns excluded from the import due to data type issues or your selections.
 
- Select Create sync. You can also select Save as draft at any step to finish later.

## Handling schema changes

The visual mapper checks the schema of your table or view on every sync run and responds based on the type of change:

 Change in your source table | 
 Sync behavior | 

 A new column is added | 
 The sync continues but new columns are not synced automatically. To include one, edit the sync and map it. | 

 A mapped column is removed | 
 The sync run fails and the sync is paused. The schema change is shown in the sync’s run details, and your notification contacts receive an email alert. | 

 A mapped column is renamed | 
 Treated as a removed column plus a new column. | 

 A column’s data type changes | 
 Not detected as a schema change. Incompatible values are reported as row errors in the sync logs. | 
...
Each column you import writes as an attribute update, and data point billing works the same as other CDI user data syncs. Excluding columns you don’t need keeps your syncs efficient. For more information, see Cloud Data Ingestion best practices.
