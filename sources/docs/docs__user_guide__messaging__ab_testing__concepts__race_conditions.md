---
url: https://www.braze.com/docs/user_guide/messaging/ab_testing/concepts/race_conditions
slug: docs__user_guide__messaging__ab_testing__concepts__race_conditions
...
- The same user is immediately targeted for a message, performs a custom event, or logs a custom attribute.
...
You can also add this delay in the Braze SDK for the specific custom event that triggers a new user to enter a Canvas.
...
Instead of using multiple endpoints, you can include the user attributes and trigger properties in a single API call using the campaign/trigger/send endpoint.
...
Use the /users/track/sync/ endpoint to record custom events and purchases and update user profile attributes synchronously. Using this endpoint to update user profiles at the same time and in a single call can help prevent potential race conditions.
