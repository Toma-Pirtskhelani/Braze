---
url: https://www.braze.com/docs/user_guide/channels/email/drag_and_drop/faq
slug: docs__user_guide__channels__email__drag_and_drop__faq
...
- On Mac: Press cmd+shift+V instead of cmd+V
 
- On Windows: Press ctrl+shift+V instead of ctrl+V
...
- Click the toggle to turn on its transparency.
...
No. You can’t save a drag-and-drop email from a campaign or Canvas as a drag-and-drop Email Template in Templates > Email Templates. Recreate the layout under Templates > Email Templates, or start from a saved template next time. For instructions, see Create an email template.

If you need a reusable HTML template instead, select Download file while editing the drag-and-drop body, open the HTML from the ZIP, and paste the markup into an HTML email template using the HTML code editor. Recheck Liquid, links, and hosted assets afterward.

For more information about where templates live, see Templates and Media.
...
- Open email global style settings and select Reset to default on the conflicting page style so the message-level color can apply.
 
- Set the color again on the block.
...
- Open your campaign or Canvas and edit the email message.
 
- Select Edit email body to open the drag-and-drop editor.
 
- Select Download file.
 
- Extract the archive to access the generated HTML.
...
You can paste that HTML into an HTML block or the HTML editor when you need low-level edits (for example, to turn off click tracking for specific links).
...
- Remove or isolate custom HTML blocks to see if the problem disappears.
 
- Check Drag-and-Drop Email Editor settings for custom fonts that may not load in all clients.
 
- In Row Properties, review column padding and widths.
 
- When you add custom HTML, prefer table-based layouts, fluid images, and total table widths that fit your email width—fixed pixel images or non-table structures often break in Outlook and other clients.
