---
url: https://www.braze.com/docs/user_guide/data/activation/events/custom_events/nested_objects
slug: docs__user_guide__data__activation__events__custom_events__nested_objects
title: "Nested objects in custom events"
description: "This article describes how to send nested JSON data as properties of custom events and purchases, and how to use those nested objects in your..."
...
The following SDK versions support nested objects:
 ...
   Swift: 5.0.0+     Web: 3.3.0+     Android: 20.0.0+  
...
The following shows how to create a Liquid template that references the nested properties requested from the previous API request.
...
- Import the nested attributes to the test user profile through the API.
 
- In your campaign or Canvas, go to Preview & Test.
 
- Select Preview as user and search for the test user. The Liquid will resolve using the actual nested attributes on that user’s profile.
...
- Create a campaign or Canvas step that targets only your test users and is triggered by (or references) the custom event with nested properties.
 
- Launch the campaign to your test audience.
 
- Log the custom event with the nested object payload to your test user’s profile (using the API or SDK).
 
- Verify that the message renders correctly with the nested property values.
