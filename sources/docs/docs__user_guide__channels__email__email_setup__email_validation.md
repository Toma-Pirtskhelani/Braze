---
url: https://www.braze.com/docs/user_guide/channels/email/email_setup/email_validation
slug: docs__user_guide__channels__email__email_setup__email_validation
...
Braze rejects certain characters and marks the address invalid. If an email bounces, Braze marks the address invalid and does not change subscription status. If the email body contains non-standard ASCII characters, Braze doesn’t send the email.
...
If the domain part contains any non-standard ASCII characters, it will need to be Punycode-encoded before being supplied to Braze.
...
The final label of the domain must be a valid top-level domain (TLD), determined by anything after the final period (.). This TLD should appear in ICANN’s TLD list. The Braze validator only checks syntax. It doesn’t catch typos or non-existent addresses.
