---
url: https://www.braze.com/docs/user_guide/brazeai/predictive_suite/predictive_churn/use_case
slug: docs__user_guide__brazeai__predictive_suite__predictive_churn__use_case
title: "Use case: Reduce churn with timely content re-engagement"
...
- In the Braze dashboard, Camila goes to Analytics > Predictive Churn.
 
- She creates a new churn prediction and names it “Churn risk in 2 weeks”.
 
- To define churn, she selects do not and the custom event stream_started, which indicates active engagement.
 
- She sets the prediction window to 14 days—meaning the model will identify users who are likely to go 14 days without starting a new stream.

- She selects a prediction audience that includes all users who have triggered relevant events in the past 30 days—giving the model enough recent behavior to learn from.
 
- She sets the prediction update schedule to weekly so scores stay current.
 
- She selects Create prediction.

The model then begins training, analyzing behaviors like recent sessions, frequency of viewing, and content interactions to surface patterns that predict drop-off. Camila gets an email an hour later that her prediction has finished training, so she opens it in Braze and checks the prediction quality score. It’s labeled “Good,” which means the model’s predictions are likely to be accurate and reliable. Confident in the model’s performance, she moves on.

## Step 2: Segment users by churn risk

After the model finishes training, Braze assigns each eligible user a Churn Risk Score between 0-100.
...
- Camila navigates to Segments in Braze.
 
- She creates a segment using the Churn Risk Score filter and selects the churn prediction she created:
...
- Camila creates a recurring campaign and enables Intelligent Timing, so each message is delivered when each individual user is most likely to engage rather than relying on a fixed day and time.
 
- She targets the “Likely to churn” segment she just created.
 
- She sets the campaign conversion event to the custom event stream_started, to track how many users actually return to view content.
 
- Camila chooses email as her primary channel—it gives her the space to highlight multiple personalized content picks in a visually rich format without too much pressure. The email includes:

- A personalized watchlist powered by AI Item Recommendations, dynamically selected from MovieCanon’s catalog
...
After a few weeks, Camila checks her campaign analytics to evaluate how well the strategy is performing.
